...
class mydsp : public dsp {
 ...
 public:
	...
	virtual void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->addHorizontalSlider("stereo", &fHslider1, FAUSTFLOAT(5e+02f), FAUSTFLOAT(0.0f), FAUSTFLOAT(1023.0f), FAUSTFLOAT(1.0f));
		ui_interface->addButton("voice", &fButton0);
		ui_interface->addHorizontalSlider("volume", &fHslider0, FAUSTFLOAT(5e+02f), FAUSTFLOAT(0.0f), FAUSTFLOAT(1023.0f), FAUSTFLOAT(1.0f));
		ui_interface->closeBox();
	}
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			output0[i0] = FAUSTFLOAT(((iSlow0) ? fElse0 : fTemp2));
			float fThen1 = fVec0[(IOTA0 - iSlow2) & 1023];
			float fElse1 = fSlow1 * fTemp0;
			output1[i0] = FAUSTFLOAT(((iSlow0) ? fElse1 : fThen1));
			IOTA0 = IOTA0 + 1;
		}
	}
	...
	FAUST_ADDHORIZONTALSLIDER("stereo", fHslider1, 5e+02f, 0.0f, 1023.0f, 1.0f);
	FAUST_ADDBUTTON("voice", fButton0);
	FAUST_ADDHORIZONTALSLIDER("volume", fHslider0, 5e+02f, 0.0f, 1023.0f, 1.0f);

	#define FAUST_LIST_ACTIVES(p) \
		p(HORIZONTALSLIDER, stereo, "stereo", fHslider1, 5e+02f, 0.0f, 1023.0f, 1.0f) \
		p(BUTTON, voice, "voice", fButton0, 0.0f, 0.0f, 1.0f, 1.0f) \
		p(HORIZONTALSLIDER, volume, "volume", fHslider0, 5e+02f, 0.0f, 1023.0f, 1.0f) \
